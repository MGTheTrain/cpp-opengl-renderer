...
namespace Mgtt::Window::Test {
    ...
    TEST_F(GlfwWindowTest, GetWindow) {
        RecordProperty("Test Description", "Checks if GetWindow method returns a non-null GLFW window pointer");
        RecordProperty("Expected Result", "No exception is thrown.");

        try {
            GLFWwindow* window = GlfwWindowTest::glfwWindow->GetWindow();
            EXPECT_NE(window, nullptr) << "The GetWindow method should return a non-null pointer.";
        }
        catch (...) {
    ...
    TEST_F(GlfwWindowTest, GetWindowSize) {
        RecordProperty("Test Description", "Checks if GetWindowSize method returns a valid width and height tuple");
        RecordProperty("Expected Result", "No exception is thrown.");
